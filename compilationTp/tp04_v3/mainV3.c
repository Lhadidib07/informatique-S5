...
UL* decouper(char *phrase,UL* phrase_decouper){
 ...
 for(int i=0;i<=strlen(phrase);i++){
    if( detecter_separateur(phrase[i])==0 && detecter_operateur(phrase[i])==0 ){
      ...
      if(( detecter_operateur(phrase[i]) || detecter_operateur_relationnel(phrase[i]) ) ){
       // printf("\n je suis dans l'insertion de operateur ");
        mot_tmp[index]=phrase[i];
        mot_tmp[index+1]='\0';
       // code=detecter_code_operateur(mot_tmp);
        printf("\n je vais inseré le %s",mot_tmp);
...
/*UL* test_AEF(UL* phrase_decouper1){
  ...
  if(phrase_decouper1== NULL){
    ...
    UL* tmp=phrase_decouper1;
    while(tmp != NULL){
     if(strlen(tmp->Lexeme)==1 && ( detecter_operateur(tmp->Lexeme) ) ){
      tmp=tmp->Suivant;
     }
     else {
       int choix=AEF_sql(tmp->Lexeme,tmp->ligne);
        switch (choix)
        {
          case 0:
          //printf("\n erreur %s dans la ligne %d ",tmp->Lexeme,tmp->ligne);
          return 0;
          break;
          case 1:
              // cherche code identifiant avec switch
              tmp->code=code_idetifiant(tmp->Lexeme);
          break;
          case 2:
            tmp->code=NOMBRE_UL;
          break;
          case 5:
              tmp->code=detecter_code_operateur(tmp->Lexeme);
          break;
          default:
            tmp->code=ERREUR;
          break;
        }
       // if(AEF_sql(tmp->Lexeme,tmp->ligne))
        //printf("\n\t LEXEME = [ %s ] \t CODE = [ %d ] \t Ligne = [ %d ] ",tmp->Lexeme,tmp->code,tmp->ligne);
        tmp=tmp->Suivant;
      }
    }
    printf("\n je suis a la ligne 380 ");
  }
  Inserer_a_la_fin(phrase_decouper1,"#",999,1);
...
int main()
{
 ...
 char phrase[500]="slect , from lhadi,dib; dib01;salam;";
 //gets(phrase);
 ...
 /*if(AEF_sql(phrase,0))
  printf("c'est bon ");
 /*if(AEF_sql(phrase,phrase_decouper))
